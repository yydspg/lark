...
// ── int64 path ───────────────────────────────────────────────────────────────
static Tensor mul_int64(const Tensor& a, const Tensor& b) {
...
// ── float64 path: explicit SIMD ──────────────────────────────────────────────
static Tensor mul_f64(const Tensor& a, const Tensor& b) {
  const size_t n = a.size();
  Tensor result(a.dtype(), n);
  ...
  for (; i + 2 <= n; i += 2) {
    ...
    vst1q_f64(pr + i, vmulq_f64(va, vb));
  }
  ...
  for (; i + 8 <= n; i += 8) {
    ...
    _mm512_storeu_pd(pr + i, _mm512_mul_pd(va, vb));
  }
  for (; i < n; ++i) pr[i] = pa[i] * pb[i];
  ...
  for (; i + 4 <= n; i += 4) {
    ...
    _mm256_storeu_pd(pr + i, _mm256_mul_pd(va, vb));
  }
  for (; i < n; ++i) pr[i] = pa[i] * pb[i];
  ...
  for (; i + 2 <= n; i += 2) {
    ...
    _mm_storeu_pd(pr + i, _mm_mul_pd(va, vb));
  }
  for (; i < n; ++i) pr[i] = pa[i] * pb[i];
...
Tensor mul(const Tensor& a, const Tensor& b) {
  if (a.dtype() != b.dtype()) throw std::runtime_error("Type mismatch in mul");
  return a.dtype() == DType::kInt64 ? mul_int64(a, b) : mul_f64(a, b);
}
