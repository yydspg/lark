...
// ── int64 path ───────────────────────────────────────────────────────────────
static Tensor sub_int64(const Tensor& a, const Tensor& b) {
...
static Tensor sub_f64(const Tensor& a, const Tensor& b) {
  const size_t n = a.size();
  Tensor result(a.dtype(), n);
  ...
  for (; i + 2 <= n; i += 2) {
    ...
    vst1q_f64(pr + i, vsubq_f64(va, vb));
  }
  ...
  for (; i + 8 <= n; i += 8) {
    ...
    _mm512_storeu_pd(pr + i, _mm512_sub_pd(va, vb));
  }
  for (; i < n; ++i) pr[i] = pa[i] - pb[i];
  ...
  for (; i + 2 <= n; i += 2) {
    ...
    _mm_storeu_pd(pr + i, _mm_sub_pd(va, vb));
  }
  for (; i < n; ++i) pr[i] = pa[i] - pb[i];
...
Tensor sub(const Tensor& a, const Tensor& b) {
  if (a.dtype() != b.dtype()) throw std::runtime_error("Type mismatch in sub");
  return a.dtype() == DType::kInt64 ? sub_int64(a, b) : sub_f64(a, b);
}
